...
var routePaths = [];
angular.module("goebu.controllers").controller('MapCtrl', function ($rootScope, $scope, $http, $timeout, $ionicPlatform, $ionicLoading, $ionicModal, $ionicSlideBoxDelegate, $ionicNavBarDelegate, $cordovaDatePicker, $cordovaToast, $cordovaActionSheet, $cordovaGeolocation, $cordovaDialogs, $cordovaNetwork, busRadar, HardwareBackButtonManager, $localstorage) {
    var rendererOptions = {
        draggable: true,
    ...
    $scope.test = function (xAndy) {
        if (originMarker && destinationMarker) {
            if (mapCanvasDiv) {
                var coordinates = holdOverlay.getProjection().fromContainerPixelToLatLng(new google.maps.Point(xAndy.pageX, xAndy.pageY - mapCanvasDivTop));
                if (coordinates) {
    ...
    bb.ax = 51.459781;
    bb.ay = 10.128727;
    var defaultBounds = new google.maps.LatLngBounds(new google.maps.LatLng(bb.ix, bb.iy), new google.maps.LatLng(bb.ax, bb.ay));
    ...
    function startCurrentLocationWatcher() {
        if (!$scope.watchPositionID) {
            ...
            $scope.watchPositionID.then(null, function (error) {
    ...
    function setUserLocationMarker(lat, lng, userTap) {
        if (previousCurrenLocationLat !== lat || previousCurrenLocationLng !== lng) {
            ...
            if (!userLocationMarker) {
                ...
                var userLocationIcon = new google.maps.MarkerImage(userLocationIconPath, new google.maps.Size(36, 36), new google.maps.Point(0, 0), new google.maps.Point(18, 18), new google.maps.Size(36, 36));
                ...
                userLocationMarker.setIcon(new google.maps.MarkerImage(userLocationIconPathWatch, new google.maps.Size(36, 36), new google.maps.Point(0, 0), new google.maps.Point(18, 18), new google.maps.Size(36, 36)));
                ...
                for (var j = 0, len2 = nData; j < len2; j++) {
                    ...
                    if (cb) {
                        if (!currentLivePosition.stalled) {
                            live_bus_bounds.extend(new google.maps.LatLng(currentLivePosition.lat, currentLivePosition.lon));
                            touched_live_bus_bounds = true;
                        }
                    }
                    if (!liveBusPositions[j]) {
                        liveBusPositions.push(new CustomBusMarker(new google.maps.LatLng(currentLivePosition.lat, currentLivePosition.lon), $scope.map, {
